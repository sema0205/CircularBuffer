...
class CircularBufferSpaceOptimized {
public:
    ...
    explicit CircularBufferSpaceOptimized (const value_type& size_buf) {
        size_ = size_buf + 1;
        data_ = alloc_traits::allocate(alloc_, size_);
        read_index_ = 1;
        write_index_ = 1;
    ...
    CircularBufferSpaceOptimized(const CircularBufferSpaceOptimized& copy_buf) {
    ...
    CircularBufferSpaceOptimized& operator=(const CircularBufferSpaceOptimized& copy_buf) {
        size_ = copy_buf.size_;
        read_index_ = 1;
        write_index_ = 1;
        elements_count_ = 0;
    ...
    void swap(CircularBufferSpaceOptimized& other_buf) {
    ...
    void push_back(const value_type& number) {
        if (elements_count_ == size_ - 1) {
            pointer new_data = alloc_traits::allocate(alloc_, (size_ - 1) * 2);
            int counter = 1;
            for (const auto el : *this) {
                new_data[counter] = el;
                counter++;
            }
            new_data[counter] = number;
            data_ = new_data;
            size_ = (size_ - 1) * 2 + 1;
            elements_count_++;
            write_index_ += 2;
    ...
    friend bool operator==(const CircularBufferSpaceOptimized& first, const CircularBufferSpaceOptimized& second) {
    ...
    const_iterator cbegin() const noexcept {
        return const_cast<CircularBufferSpaceOptimized const&>(*this).begin();
    }

    const_iterator cend() const noexcept {
        return const_cast<CircularBufferSpaceOptimized const&>(*this).end();
    }
